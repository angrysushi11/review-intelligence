...
test("the DoubleDash marketplace resolves the public Review Intelligence plugin", async () => {
  ...
  const plugin = JSON.parse(await readFile(new URL("plugins/review-intelligence/plugin.json", root), "utf8"));
  const portableMcp = JSON.parse(await readFile(new URL("plugins/review-intelligence/mcp.json", root), "utf8"));
  const compatibilityPlugin = JSON.parse(await readFile(new URL("plugins/review-intelligence/.codex-plugin/plugin.json", root), "utf8"));
  const compatibilityMcp = JSON.parse(await readFile(new URL("plugins/review-intelligence/.mcp.json", root), "utf8"));
  ...
  assert.doesNotMatch(readme, /reviews\.doubledash\.me\/mcp|doubledash\.me\/tools\/review-intelligence/);
});
